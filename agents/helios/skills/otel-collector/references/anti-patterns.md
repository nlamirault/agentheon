...
This reference catalogs known anti-patterns across collector configuration, metrics design, Kubernetes deployments, AI agent instrumentation, and OTTL transforms. The top anti-patterns are also kept inline in `SKILL.md`; all patterns are here with explanations and mitigations.
...
`pprof` (port 1777) and `zpages` (port 55679) expose internal profiling and pipeline state. Binding them to all interfaces in a shared network allows any workload to extract sensitive runtime data.
...
**Fix:** Use a `ClusterIP` or `LoadBalancer` Service instead. Reserve `hostPort` for DaemonSet node-agent patterns.
...
### `deltatocumulative` / `cumulativetodelta` without checking source temporality
...
**Fix:** Confirm source SDK temporality, backend expectations, restart tolerance, and replica routing before enabling temporality conversion processors.

---

### Using delta temporality with backends that expect cumulative

Some backends (VictoriaMetrics, OpenTSDB) silently drop delta-temporality metrics or misinterpret them as cumulative.
...
These identifiers are unbounded (a new value per agent invocation). Using them as metric label values creates cardinality explosions.
...
These settings log raw prompt and completion text. In shared or production environments this can expose user PII, secrets embedded in prompts, and proprietary code.
...
**Fix:** Always include metrics and logs pipelines alongside any traces pipeline. Check the AI agent support matrix in [ai-agents.md](../../../otel-instrumentation/references/ai-agents.md).

---

### Hard-coding `gen_ai.token.type` to only `input`/`output` values
