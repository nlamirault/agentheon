...
# Derives every profile from the single source of truth — agents/*.md frontmatter
...
Derives every profile from agents/*.md frontmatter and installs it under
...
Env overrides:
  ...
  HERMES_RESERVED replacement for reserved "hermes"   (default: hermes-agent)
...
for file in "${AGENTS_DIR}"/*.md; do
  ...
  [[ "$slug" == "hermes" ]] && { slug="$HERMES_RESERVED"; echo "${INFO} '$name' → profile '$slug' (name 'hermes' is reserved)"; }
...
# SPDX-License-Identifier: Apache-2.0
#
# Generated by agentheon.sh from agents/${slug}.md — do not edit by hand.
...
YAML
  ...
  if [[ "$DRY_RUN" == 1 ]]; then
  ...
  else
    {
      ...
      echo "#"
      echo "# Portable profile descriptor generated by agentheon.sh from agents/${slug}.md."
  ...
  gen="$(mktemp)"
  {
    echo "<!-- AGENTHEON:BEGIN — generated from agents/${slug}.md by agentheon.sh; do not edit inside this block, it is overwritten. -->"
    echo "# ${name} — ${title}"
    echo
    echo "**Domain:** ${domain}"
